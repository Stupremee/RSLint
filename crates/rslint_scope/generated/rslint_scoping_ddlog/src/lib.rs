...
pub static RELIDMAPC: ::once_cell::sync::Lazy<::fnv::FnvHashMap<RelId, &'static ::std::ffi::CStr>> =
    ::once_cell::sync::Lazy::new(|| {
        ...
        map.insert(32, ::std::ffi::CStr::from_bytes_with_nul(b"__Prefix_0\0").expect("Unreachable: A null byte was specifically inserted"));
        map
    });
...
pub fn relval_from_record(rel: Relations, _rec: &differential_datalog::record::Record) -> ::std::result::Result<DDValue, String> {
    match rel {
        ...
        Relations::__Prefix_0 => {
            Ok(<::types::ddlog_std::tuple3<u32, ::types::internment::Intern<::types::Pattern>, ::types::internment::Intern<String>>>::from_record(_rec)?.into_ddvalue())
        }
    }
}
...
pub fn prog(__update_cb: Box<dyn CBFn>) -> Program {
    ...
    let __Prefix_0 = Relation {
                         ...
                         rules:        vec![
                             /* __Prefix_0[((stmt: bit<32>), (pat: internment::Intern<Pattern>), (name: internment::Intern<string>))] :- LetDecl[(LetDecl{.stmt_id=(stmt: bit<32>), .pattern=(ddlog_std::Some{.x=(pat: internment::Intern<Pattern>)}: ddlog_std::Option<IPattern>), .value=(_: ddlog_std::Option<ExprId>)}: LetDecl)], ((SinglePattern{.name=(var name: internment::Intern<string>)}: Pattern) = ((internment::ival: function(internment::Intern<Pattern>):Pattern)(pat))). */
                             Rule::CollectionRule {
                                 description: "__Prefix_0[((stmt: bit<32>), (pat: internment::Intern<Pattern>), (name: internment::Intern<string>))] :- LetDecl[(LetDecl{.stmt_id=(stmt: bit<32>), .pattern=(ddlog_std::Some{.x=(pat: internment::Intern<Pattern>)}: ddlog_std::Option<IPattern>), .value=(_: ddlog_std::Option<ExprId>)}: LetDecl)], ((SinglePattern{.name=(var name: internment::Intern<string>)}: Pattern) = ((internment::ival: function(internment::Intern<Pattern>):Pattern)(pat))).".to_string(),
                                 rel: Relations::LetDecl as RelId,
                                 xform: Some(XFormCollection::FilterMap{
                                                 description: "head of __Prefix_0[((stmt: bit<32>), (pat: internment::Intern<Pattern>), (name: internment::Intern<string>))] :- LetDecl[(LetDecl{.stmt_id=(stmt: bit<32>), .pattern=(ddlog_std::Some{.x=(pat: internment::Intern<Pattern>)}: ddlog_std::Option<IPattern>), .value=(_: ddlog_std::Option<ExprId>)}: LetDecl)], ((SinglePattern{.name=(var name: internment::Intern<string>)}: Pattern) = ((internment::ival: function(internment::Intern<Pattern>):Pattern)(pat)))." .to_string(),
                                                 fmfun: &{fn __f(__v: DDValue) -> Option<DDValue>
                                                 {
                                                     ...
                                                     Some((::types::ddlog_std::tuple3((*stmt).clone(), (*pat).clone(), (*name).clone())).into_ddvalue())
                                                 }
                                                 __f},
                                                 next: Box::new(None)
                                             })
                         ...
                         arrangements: vec![
                             Arrangement::Map{
                                name: r###"((_0: bit<32>), (_: internment::Intern<Pattern>), (_: internment::Intern<string>)) /*join*/"###.to_string(),
                                 afun: &{fn __f(__v: DDValue) -> Option<(DDValue,DDValue)>
                                 {
                                     let __cloned = __v.clone();
                                     match unsafe {< ::types::ddlog_std::tuple3<u32, ::types::internment::Intern<::types::Pattern>, ::types::internment::Intern<String>>>::from_ddvalue(__v) } {
    ...
    let NameInScope = Relation {
                          ...
                          rules:        vec![
                              ...
                              /* NameInScope[(NameInScope{.name=name, .scope=scope, .declared_in=(ddlog_std::Right{.r=stmt}: ddlog_std::Either<StmtId,FuncId>)}: NameInScope)] :- __Prefix_0[((stmt: bit<32>), (pat: internment::Intern<Pattern>), (name: internment::Intern<string>))], Statement[(Statement{.id=(stmt: bit<32>), .kind=(_: StmtKind), .scope=(scope: bit<32>), .span=(_: Span)}: Statement)]. */
                              Rule::ArrangementRule {
                                  description: "NameInScope[(NameInScope{.name=name, .scope=scope, .declared_in=(ddlog_std::Right{.r=stmt}: ddlog_std::Either<StmtId,FuncId>)}: NameInScope)] :- __Prefix_0[((stmt: bit<32>), (pat: internment::Intern<Pattern>), (name: internment::Intern<string>))], Statement[(Statement{.id=(stmt: bit<32>), .kind=(_: StmtKind), .scope=(scope: bit<32>), .span=(_: Span)}: Statement)].".to_string(),
                                  arr: ( Relations::__Prefix_0 as RelId, 0),
                                  xform: XFormArrangement::Join{
                                             description: "__Prefix_0[((stmt: bit<32>), (pat: internment::Intern<Pattern>), (name: internment::Intern<string>))], Statement[(Statement{.id=(stmt: bit<32>), .kind=(_: StmtKind), .scope=(scope: bit<32>), .span=(_: Span)}: Statement)]".to_string(),
                                             ffun: None,
                                             arrangement: (Relations::Statement as RelId,1),
                                             jfun: &{fn __f(_: &DDValue ,__v1: &DDValue,__v2: &DDValue) -> Option<DDValue>
                                             {
                                                 let (ref stmt, ref pat, ref name) = match *unsafe {<::types::ddlog_std::tuple3<u32, ::types::internment::Intern<::types::Pattern>, ::types::internment::Intern<String>>>::from_ddvalue_ref(__v1) } {
                                                     ::types::ddlog_std::tuple3(ref stmt, ref pat, ref name) => ((*stmt).clone(), (*pat).clone(), (*name).clone()),
                                                     _ => return None
                                                 };
